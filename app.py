...
CLOUD_SERVICES_DISABLED_FLAG = os.path.join(DATA_DIR, "cloud-services-disabled")


def apply_cloud_services_preference():
    """Enforce cloud services preference on startup.

    This ensures the setting persists across OTA updates.
    ...
    # User disabled cloud services - ensure they stay disabled
    ...
    # Backup mender.conf if it exists (may have been regenerated by OTA)
...
def cloud_services_status():
    ...
    # Flag file is the source of truth (survives OTA updates)
    disabled = os.path.exists(CLOUD_SERVICES_DISABLED_FLAG)
...
def cloud_services_toggle():
    ...
    try:
        if enabled:
            # Remove flag file first (source of truth)
            if os.path.exists(CLOUD_SERVICES_DISABLED_FLAG):
                os.remove(CLOUD_SERVICES_DISABLED_FLAG)

            # Restore mender.conf (if backed up)
            if os.path.exists(MENDER_CONF_BACKUP_PATH):
                shutil.move(MENDER_CONF_BACKUP_PATH, MENDER_CONF_PATH)
            ...
            # Create flag file (source of truth - survives OTA updates)
            os.makedirs(DATA_DIR, exist_ok=True)
            with open(CLOUD_SERVICES_DISABLED_FLAG, 'w') as f:
                f.write('')
