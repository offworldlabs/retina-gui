...
class TestIndexRoute:
    ...
    def test_index_shows_config_form(self, app_client):
        """Index should show config form when retina-node is installed."""
        response = app_client.get('/')
        assert response.status_code == 200
        assert b'Capture Settings' in response.data
        assert b'Sample Rate' in response.data
        assert b'Apply Changes' in response.data

    def test_index_shows_values_from_user_yml(self, app_client):
        """Form should display values from user.yml."""
        response = app_client.get('/')
        assert response.status_code == 200
        # Check sample config values are in the HTML
    ...
    def test_index_no_retina_node(self, app_client_no_retina):
        """Should show message when retina-node not installed."""
        response = app_client_no_retina.get('/')
        assert response.status_code == 200
